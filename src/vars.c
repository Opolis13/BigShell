...
is_valid_varname(char const *name)
{
  ...
  if (!isalpha(*name) && *name != '_') {
    return 0;  // If the first letter is not a letter or underscore return 0 value for not valid
  }

  while(*++name) {
    if (!isalnum(*name) && *name != '_') {
      return 0; // Invalid if not letter or underscore
    }
  }
...
{
  /* Argument validation before tail-calling internal
