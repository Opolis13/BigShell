 ...
 *
 * It is an error if too many arguments are provided(ok?), or if the chdir operation
 * fails
 */
static int
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  } else if (cmd->word_count == 2) { // When one argument is provided use the variable as the directory

      target_dir = cmd->words[1];

    } else if (cmd->word_count > 2) { // When too many arguments are provided to the function return an error and -1
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: Too many arguments provided.\n");
    return -1;
    }

  if (chdir(target_dir) != 0) { // check if the chdir operation is successful, if fail return -1
      dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: error changing directory\n");
      return -1;
  }
...
{
  int exit_value = params.status; // Should default to the last terminated foreground command

  if (cmd->word_count == 1) {
    params.status = exit_value; // use the default val if no num provided
  } else if (cmd->word_count == 2) {
    params.status = atoi(cmd->words[1]); // convert the string provided to a int using atoi and set the param status to the new int val
  } else {
      dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: Too many arguments provided.\n");
      return -1;
    }
...
{
  for (size_t i = 1; i < cmd->word_count; ++i) { // loop through the argument to unset the variables
    vars_unset(cmd->words[i]); // through the vars_unset function declared in vars we unset each variables in the argument
  }
  return 0;
}
