  ...
  for (;;) {
    ...
    if (res < 0) {
      ...
      if (errno == ECHILD) {
        ...
        if (WIFEXITED(status)) {
          /* Set params.status to the correct value ok?*/
    ...
    /* A child exited, but others remain. */
  }
...
wait_on_bg_jobs()
{
  ...
  for (size_t i = 0; i < job_count; ++i) {
    ...
    for (;;) {
       ...
       */
      int status;
      pid_t pid = waitpid(-pgid, &status, WNOHANG | WUNTRACED); // WNOHANG Returns immediately instead of waiting for the child’s status. 
      if (pid == 0) {
