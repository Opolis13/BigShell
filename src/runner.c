...
do_variable_assignment(struct command const *cmd, int export_all)
{
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    vars_set(a->name, a->value); // use vars_set from vars.h to set the variable to the value
...
get_io_flags(enum io_operator io_op)
{
  ...
  switch (io_op) {
    case OP_LESSAND: /* <& */
    case OP_LESS:    /* < */
      flags = O_RDONLY;     /* Open for reading*/
      break;
      
    case OP_GREATAND: /* >& */
    case OP_GREAT:    /* > */
      flags = O_WRONLY | O_CREAT | O_EXCL;      /* We dont want to overwrite the file so open with exclusive */
      break;                              // Ensure that this call creates the file: if this flag is specified in conjunction with O_CREAT
    case OP_DGREAT: /* >> */
      flags = O_WRONLY | O_CREAT | O_APPEND;    /* Open the file for writing, if it doesnt exist create the file and then append if it exists*/
      break;
    case OP_LESSGREAT: /* <> */
      flags = O_RDWR | O_CREAT;       /* Open for read and write, create if doesnt exist*/
      break;
    case OP_CLOBBER: /* >| */
      flags = O_WRONLY | O_CREAT | O_TRUNC;     /* Use WRONLY or RDWR before using TRUNC*/
      break;
  }
  return flags;
}
...
move_fd(int src, int dst)
{
  ...
  if (dup2(src,dst) < 0) return -1; // dup2 will duplicate the src to the dst and close the src file if fails, wont change(?)
  /* TODO close src -- ok?*/
  if (close(src) < 0) return -1;
  return dst;
}
...
do_io_redirects(struct command *cmd)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    if (0) {
    err: /* Anything that can fail should jump here.*/
      gprintf("Unable to complete");
      status = -1;
    }
  }
  return status;
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
     ...
     *
     * Update upstream_pipefd initializer to get the (READ) side of the pipeline saved from the previous command
     */
    ...
    if (is_pl && pipe(pipe_fds) < 0) goto err;
    ...
    if (child_pid == 0) {
      if (is_builtin) {
         ...
         */
        if (upstream_pipefd >= 0 && dup2(upstream_pipefd, STDIN_FILENO) < 0) {
          exit(1);
        }

        if (downstream_pipefd >= 0 && dup2(downstream_pipefd, STDOUT_FILENO) < 0) {
          exit(1);
        }
